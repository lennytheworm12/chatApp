...
const createFakeSocket = (
    userId: string,
    overrides: Partial<{ cookie: string }> = {},
) => ({
    data: { userId },
    handshake: {
        auth: {},
...
describe('Socket authentication', () => {
    ...
    it('ignores handshake.auth.token and requires the jwt cookie', () => {
        const authToken = jwt.sign({ userId: otherUserId }, JWT_SECRET);
        const cookieToken = jwt.sign({ userId: validUserId }, JWT_SECRET);
        const socket = createFakeSocket('', { cookie: `jwt=${cookieToken}` }) as unknown as ChatSocket;
        socket.handshake.auth = { token: authToken };

        expect(getSocketToken(socket)).toBe(cookieToken);
    });

    it('rejects connections that only present a handshake.auth.token', () => {
        const token = jwt.sign({ userId: validUserId }, JWT_SECRET);
    ...
    it('rejects connections with an invalid token', () => {
        const socket = createFakeSocket('', { cookie: 'jwt=not-a-jwt' }) as unknown as ChatSocket;
        const next = jest.fn();
    ...
    it('rejects connections when JWT_SECRET is not configured', () => {
        ...
        try {
            const socket = createFakeSocket('', { cookie: 'jwt=any-token' }) as unknown as ChatSocket;
            const next = jest.fn();
...
describe('Socket message handling', () => {
    ...
    it('emits messageError when persistence fails', async () => {
        ...
        jest.spyOn(UserModel, 'exists').mockResolvedValueOnce({ _id: otherUserId } as never);
    ...
    it('rejects a nonexistent recipient via messageError and ack without persisting', async () => {
        const socket = createFakeSocket(validUserId) as unknown as ChatSocket;
        const emit = socket.emit as jest.Mock;
        const io = createFakeIo();
        const ack = jest.fn();
        ...
        expect(emit).toHaveBeenCalledWith('messageError', { error: 'Recipient does not exist' });
        expect(ack).toHaveBeenCalledWith({ error: 'Recipient does not exist' });
        expect(io.to).not.toHaveBeenCalled();
        expect(await MessageModel.countDocuments()).toBe(0);
    });

    it('handles a null populate result without crashing and reports a safe error', async () => {
        const socket = createFakeSocket(validUserId) as unknown as ChatSocket;
        const emit = socket.emit as jest.Mock;
        const io = createFakeIo();
        const ack = jest.fn();
...
describe('Socket wiring', () => {
    ...
    it('rejects socket handshakes from unlisted browser origins and accepts listed origins', async () => {
        ...
        try {
            ...
            const denied = await fetch(
                ...
                { headers: { Origin: 'http://evil.example', Connection: 'close' } },
            );
            await denied.text();
            // engine.io converts a failing CORS middleware into a BAD_REQUEST
            // (400) handshake rejection; the connection must not succeed.
